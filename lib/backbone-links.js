(function (global, factory) {
	...
	_.extend(Link.prototype, Backbone.Events, {
		...
		attach: function() {
			...
			// update the link anytime the collection changes
		...
		detach : function() {
			...
			// stop listening to the collection and model
		...
		update: function() {
			...
			// clean up if the new type changed
			if (type !== this.type) this.clean();
			...
			switch (type) {
				...
				case Link.MODEL_LINK:
					...
					// if the model exists now, we set it
					if (model != null) {
						this.setValid(true);
