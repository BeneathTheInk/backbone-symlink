(function (global, factory) {
	...
	var model_methods = {
		...
		symlink: function(attr, col, options) {
		...
		toJSON: function(_super) {
			return function(options) {
				...
				// convert symlinks
				_.each(this._symlinks, function(link, attr) {
					if (_.has(data, attr)) data[attr] = link.value;
				});
	...
	Symlink.configure = function(Backbone) {
		var mproto = Backbone.Model.prototype;
		_.each(model_methods, function(fn, name) {
			if (_.has(mproto, name)) fn = fn(mproto[name]);
			mproto[name] = fn;
		});

		Backbone.Symlink = Symlink;
		return Backbone;
	}

	// autoconfigure and export the current Backbone instance
	return Symlink.configure(Backbone);
}));